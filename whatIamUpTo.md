...
- [x] Clean up vars, organise secrets
...
Beware how you apply the secret because that's not on the repo.
...
##### 2. Make Kubernetes secrets Terraform variables

Important to note that velero variable needs to be base64encoded

I tried to figure a way to get that variable without having to use the velero command, I figure when doing this
```bash
 kubectl get secrets/cloud-credentials -n velero --template={{.data.cloud}} | base64 -d
```
...
```
That is is nothing else than a base64 encoded version. So the way to get that var without velero is doing a simple

```bash
cat /tmp/awscredentials | base64
```
Being the file I pointed there the file containing the AWS credentials. 

##### Gets stuck destroying

Doesn't seem to happen after changing a few depends_on but added info on README.md of how to fix this if ever happened. 
